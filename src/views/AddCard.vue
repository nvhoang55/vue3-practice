    ...
    <div class="card mb-5">
      <main class="form-add">
        <Form v-slot="{ errors, meta, isSubmitting  }" :initial-values="initialData" :validation-schema="validateSchema"
          ...
          <!-- section Display all errors-->
          <ul v-if="!meta.valid && meta.dirty" class="list-group errors-display">
            <li v-for="(message, field) in errors" :key="field" class="list-group-item list-group-item-danger">
              {{ message }}
            </li>
          </ul>
          ...
          <div class="input-group">
            <Field v-model="name" aria-label="Name"
                   class="form-control"
                   name="name"
          ...
          <Field v-model="imageURL"
                 class="form-control"
                 name="imageURL"
                 placeholder="Image source" type="url"/>
          ...
          <div v-for="(item, index) in positions" :key="index" class="input-group positions mb-1">
            <Field
                :name="`positions[${index}].name`"
                aria-label="Position"
                class="form-control position-name"
...
export default {
  ...
  setup()
  {
    ...
    const validateSchema = object({
      ...
      imageURL: string().required().label("Image URL"),
      positions: array().of(object({
        name: string().required().label("Position name"),
        tenure: array().of(number().typeError("Must be a number").positive("Must > 0").integer().required().label("Tenure year"))
      })).required(),
      bio: object({
        born: object({
          year: number()
              .typeError("Born year must be a number")
              .positive("Born year must > 0").integer()
              .required()
              .label("Born year"),
          place: string()
        }),
        died: object({
          year: number()
              .typeError("Died year must be a number")
              .positive("Died year must > 0").integer()
              .required("Required")
              .label("Died year"),
          place: string()
        }).test(
            "not-after-born",
            "Died year can't be after born year",
            (value, context) =>
            {
              return context.parent.born.year < value.year;
            }),
        burial: object({
          date: string(),
          place: string()
  ...
  mounted()
  {
    ...
    // Iterate over elements and add each one of them a `click` listener
    inputs.forEach(input =>
    {
      //select all input text when click in
